...
export default function IntentPanel({ onInfo, defaultWidth = 320 }) {
  ...
  // Filter pipeline (client-side; we'll add a search view later for backend search)
  ...
  useEffect(() => {
    if (isAtBottom && outerRef.current) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      <div style={{
        ...
        {canScrollUp && (
          <button
            onClick={() => setWindowStart(w => Math.min(w + MAX_RENDERED, total - MAX_RENDERED))}
            style={{
              margin: '4px 10px',
              fontSize: 9,
              padding: '4px 0',
