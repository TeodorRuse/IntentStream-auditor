...
import {
  Activity, Network, Link2, BarChart3, Search,
...
const styles = {
  ...
  brand: { padding: '18px 16px 16px', borderBottom: '1px solid var(--line)' },
  brandLogo: {
  ...
  navSectionLine: { flex: 1, height: 1, background: 'var(--line)', opacity: 0.6 },
  status: { borderTop: '1px solid var(--line)', padding: '12px 16px' },
  statusRow: {
    display: 'flex', alignItems: 'center', gap: 8,
    fontFamily: 'var(--font-display)', fontSize: 10,
    letterSpacing: '0.08em', marginBottom: 4,
  },
  statusMeta: { fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--ink-faint)' },
}
...
const ITEM_BASE = {
  ...
  cursor: 'pointer', gap: 10, textDecoration: 'none',
  ...
  return (
    <aside style={styles.root}>
      <div style={styles.brand}>
        ...
        <div style={styles.brandSub}>v2.0 · audit</div>
      </div>
      ...
      <nav style={styles.nav}>
        ...
        onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--pink-dim)'; e.currentTarget.style.color = 'var(--pink)' }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--line)'; e.currentTarget.style.color = 'var(--ink-dim)' }}>
          ...
          <span style={{ flex: 1 }}>palette</span>
          <kbd style={{
